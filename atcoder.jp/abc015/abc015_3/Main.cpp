...
void dfs(int h, int w,vector<string> &field, vector<vector<int>> &seen, int &count,int H, int W){
    //cout << h << " " << w <<" ";
    seen[h][w] = 1;
    for(int i=0; i<4; i++){
        int nh = h + dh[i];
        ...
        if(field[nh][nw]=='.')count++;
        dfs(nh,nw,field,seen,count,H,W);
    }
}
    ...
    for(int i=0; i<K; i++){
        for(int j=0; j<K; j++){
            for(int k=0; k<K; k++){
                for(int l=0; l<K; l++){
                    for(int m=0; m<K; m++){
                        int a = (T[0][i] ^ T[1][j] ^ T[2][k] ^ T[3][l] ^ T[4][m]);
                        if(a == 0){
                            cout << "Found" << endl;
                            return 0;
                        }
