...
int main() {
    ...
    for(int i=1; i<=N; i++){
        int friend_size = F.size(i) - 1;
        int friend_dir = f_dir[i].size();
        int ans = friend_size - friend_dir;
        for(auto value: block[i]){
            if(F.issame(i,value) && !f_dir[i].count(value)){
                ans--;
            }
        }
        cout << ans << " ";
