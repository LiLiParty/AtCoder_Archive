...
int main() {
    ...
    for(int i=0; i<N; i++){
        if(i == 0){
            ...
            if(A[i]>=0) P_num.push_back(A[i]);
            else N_num.push_back(A[i]);
        }
    }
    /*
    for(auto a: P_num){
    ...
    */
    ...
    vector<int> ans;
    for(int i=1; i<P_num.size(); i++){
        ans.push_back(N_num[0]);
        ans.push_back(P_num[i]);
        N_num[0] -= P_num[i];
    }
    for(int i=0; i<N_num.size(); i++){
        ans.push_back(P_num[0]);
        ans.push_back(N_num[i]);
        P_num[0] -= N_num[i];
    }
    cout << P_num[0] << endl;
