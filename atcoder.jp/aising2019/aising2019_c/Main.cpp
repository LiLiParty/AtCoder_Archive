...
void dfs(int h, int w,vector<string> &field, vector<vector<int>> &seen, ll &count1,ll &count2,int H, int W){
    seen[h][w] = 1;
    if(field[h][w]=='.')count1++;
    if(field[h][w]=='#')count2++;
    for(int i=0; i<4; i++){
        ...
        dfs(nh,nw,field,seen,count1,count2,H,W);
    }
}
...
int main() {
    ...
    for(int i=0; i<H; i++){
        for(int j=0; j<W; j++){
            ...
            ll count1 = 0;
            ll count2 = 0;
            dfs(i,j,field,seen,count1,count2,H,W);
            //cout << i << " " << j << " " << count << endl;
            ans += count1*count2;
        }
    }
