...
int main() {
    ...
    while(1){
        if(!m.count(right)){
            string res = inquiry(right);
            if(res == "Vacant") return 0;
            m[right] = res;
        }
        if(!m.count(left)){
            string res = inquiry(left);
            if(res == "Vacant") return 0;
            m[left] = res;
        }
        if(!m.count( (right+left)/2 ) ){
            string res = inquiry((right+left)/2);
            if(res == "Vacant") return 0;
            m[(right+left)/2] = res;
        }
        int mid = (right+left)/2;
        if(     
        ((right-mid)%2==1 && m.at(right) == m.at(mid)) ||
        ((right-mid)%2==0 && m.at(right) != m.at(mid))
        ){ 
            left = mid;
            continue;
        }
        if(     
        ((mid-left)%2==1 && m.at(mid) == m.at(left)) ||
        ((mid-left)%2==0 && m.at(mid) != m.at(left))
        ){ 
            right = mid;
            continue;
        }
