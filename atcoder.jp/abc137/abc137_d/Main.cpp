...
int main(){
    int N,M;
    cin >> N >> M;
    priority_queue<pair<ll,ll>, vector<pair<ll,ll>>, greater<pair<ll,ll>>> pA;
    vector<ll> V(N);
    for(int i=0; i<N; i++){
        int A,B;
        cin >> A >> B;
        pA.push(make_pair(A,i));
        V.at(i) = B;
    }
    ...
    for(int day=M; day>=0 ; day-=1){
        
        if(count <N){
            while(pA.top().first <= M-day){
                qB.push(V[pA.top().second]);
                pA.pop();
                count ++;
                if(count>=N) break;
                if(pA.empty()) break;
